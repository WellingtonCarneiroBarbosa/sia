...
class UserActivityReportController extends Controller
{
    ...
    static function getLogs($data, $userID)
    {
        ...
        $schedules_log = ScheduleLog::where('user_id', $userID)->where('created_at', '>=', $data['start'])->where('created_at', '<=', $data['end'])->take($limit)->get();
        $customers_log = CustomerLog::where('user_id', $userID)->where('created_at', '>=', $data['start'])->where('created_at', '<=', $data['end'])->take($limit)->get();
        $places_log = PlaceLog::where('user_id', $userID)->where('created_at', '>=', $data['start'])->where('created_at', '<=', $data['end'])->take($limit)->get();
        $users_log = UserLog::where('user_action_id', $userID)->where('created_at', '>=', $data['start'])->where('created_at', '<=', $data['end'])->take($limit)->get();
